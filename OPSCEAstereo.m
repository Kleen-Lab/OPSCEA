function OPSCEAstereo(pt,sz,showlabels,jumpto)
...
for i=frametimpoints; 
    ...
    spnums={[1:35],[36:70],[101:135],[136:170]};
    for subp=1:4; 
    subplot(2,100,spnums{subp})
    for j=1:size(plt,1); 
    switch upper(plottype{j,1})
      case 'SURFACE' % plotting surfaces only
          siiide=plt{j,find(strcmpi(fields_PLOT,'surfaces'))}; siiide=siiide(1);
          
          if subp>1
          if ~strcmpi(siiide,'L'); glass1=ctmr_gauss_plot_edited(Rcrtx,em(nns,:),w8s(nns),S.cax,0,S.cm,S.gsp); alpha(glass1,.1); end
          if ~strcmpi(siiide,'R'); glass2=ctmr_gauss_plot_edited(Lcrtx,em(nns,:),w8s(nns),S.cax,0,S.cm,S.gsp); alpha(glass2,.1); end
      ...
      case 'DEPTH' %plot depth electrode with parallel slice (plus surface behind it)
        ...
        elseif ~isempty(eNID); I.em=em; I.w8s=w8s; I.nns=nns; sliceinfo(j).depthlabels=depthlabels{j}; 
          OPSCEAsurfslice2023(pt,S.sliceplane,em(eNID,:),w8s(eNID),opsceadatapath,[],S.cax,S.cm,S.gsp,j,isfirstframe&subp==1,siiide)
          ...
          %plot3(em(eNID,1),em(eNID,2),em(eNID,3),'-','color',depthcolor{j},'linewidth',1); % depth probe (line between electrodes) %####### NEW (color)   
          plot3(em(eNID,1),em(eNID,2),em(eNID,3),'-','color','k','linewidth',1); % depth probe (line between electrodes) %####### NEW (color)   
          plot3(em(eNID,1),em(eNID,2),em(eNID,3),'.','color','k','markersize',12); % depth electrodes (dots) %####### NEW (color)
          ...
          LBxyz=em(eNID(end),:)+2*(em(eNID(end),:)-em(eNID(end-1),:));
          if subp<5; text(LBxyz(1,1),LBxyz(1,2),LBxyz(1,3),ttl,'HorizontalAlignment','center','VerticalAlignment','middle','color','k','fontweight','bold','fontsize',10); end %depthcolor{j}
          
          colormap(gca,S.cm)

        end
    end; pause(.5) 
    end; %done with loop through plot rows
    %if siiide=='R'; xlim([0 max(xlim)]); elseif siiide=='L'; xlim([min(xlim) 0]); end
    delete(findall(gca,'type','light')); 
    litebrain('a',.6); 
    litebrain('p',.6); 
    litebrain('l',.6); 
    litebrain('r',.6); 
    %axis(axislim); 
    if subp>1; zoom(2.5); end
    view(viewangle+90*(subp-1),30); 

    end %done with subplots
...
vidfilename = ['/Volumes/KLEEN_DRIVE/OPSCEA/OPSCEADATA/' pt '_' sz '_' jkdatetime];
