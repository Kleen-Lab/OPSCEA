function OPSCEAsurfslice(subject,orientation,elecs,weights,subj_dir,fs_dir,cax,CM,gsp,j,isfirstframe,siiide)
    ...
    if siiide=='L'; alphamap(:,128:end)=0; elseif siiide=='R'; alphamap(:,1:127)=0; end
    ...
    surface(sliceinfo(j).XX,sliceinfo(j).YY,sliceinfo(j).ZZ,'CData',sliceimage,'EdgeColor','none','FaceColor','texturemap','FaceAlpha','texturemap','EdgeAlpha',0,'AlphaData',double(alphamap)*.5,'AlphaDataMapping','none','specularexponent',5);
    shading flat
