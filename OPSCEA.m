...
szparams = fullfile(szpath, 'clip_params.mat');
...
load(szparams, 'vidstart', 'vidstop');
S.VIDperiod=[vidstart vidstop];
load(szparams, 'blstart', 'blstop')
S.BLperiod=[blstart blstop];

%transform, scaling, and display options
load(szparams, 'llw', 'iceeg_scale', 'fps', 'cax', 'gsp', 'cm', 'iceegwin', 'marg', 'slicebright');
S.llw=llw;
...
sliceinfo.viewangle=zeros(nplots,3); sliceinfo.azel=[]; sliceinfo.corners=[]; loaf.isR=isR; loaf.isL=isL;
